...
class DQN:
    def __init__(self, actor, target_actor, gamma, batch_size, replay_buffer_size, train_mode, update_time):
    ...
    def act(self, obs):
        ...
        value_action = self.actor(state)

        if self.train_mode:
            if np.random.random() > self.epsilon:
                action = np.argmax(value_action.cpu().data.numpy())
            else:
                action = np.random.choice(np.arange(self.actor.action_len))
        else:
            action = np.argmax(value_action.cpu().data.numpy())

        return action, action

    def epislon_decay(self, eps_end=0.01, eps_decay=0.995):
